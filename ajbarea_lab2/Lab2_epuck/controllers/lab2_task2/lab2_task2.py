...
def backwardsCircle(first_left_speed, first_right_speed):
    ...
    while (
        ...
        and ((imu.getRollPitchYaw()[2] * (180 / pi)) % 360) != 360
        and start_time > 0
    ):
        leftMotor.setVelocity(-first_left_speed)
        rightMotor.setVelocity(-first_right_speed)
        getIMUDegrees()
        if math.isclose(imu.getRollPitchYaw()[2], 0.1, abs_tol=0.01):
            return True
...
def circles(R1, R2, X, Y):
    ...
    if (
        first_left_speed > 6.28
        or first_left_speed < -6.28
        or first_right_speed > 6.28
        or first_right_speed < -6.28
    ):
        exitError()
    if (
        second_left_speed > 6.28
        or second_left_speed < -6.28
        or second_right_speed > 6.28
        or second_right_speed < -6.28
    ):
        exitError()

    # Run backwards circle - return value not used
    backwardsCircle(first_left_speed, first_right_speed)

    leftMotor.setVelocity(second_left_speed)
    rightMotor.setVelocity(second_right_speed)
    while (
        robot.step(timestep) != -1
        and (imu.getRollPitchYaw()[2] * (180 / pi)) % 360 != 0
        and start_time > 1
    ):
        leftMotor.setVelocity(second_left_speed)
        rightMotor.setVelocity(second_right_speed)
        getIMUDegrees()
        if math.isclose(imu.getRollPitchYaw()[2], (pi / 2), abs_tol=0.02):
